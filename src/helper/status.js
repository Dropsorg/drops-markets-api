...
const updateProtocolStatusData = async (allMarkets, network) => {
  console.log('====>updateProtocolStatusData started: ', new Date());
  ...
  try {
    ...
    if (network === 1) {
      await fs.writeFileSync('src/data/status.json', JSON.stringify(data));
    } else {
      await fs.writeFileSync('src/data/status-test.json', JSON.stringify(data));
    }
  } catch (err) {
    console.log(`updateProtocolStatusData failed, error=${err}`);
  }

  console.log('====>updateProtocolStatusData end: ', new Date());
