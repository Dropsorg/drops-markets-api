...
const updateMarketData = async (network) => {
  ...
  try {
    ...
    const comptrollerContracts = comptrollers.map(
      (comptroller) =>
        new Contract(comptroller.address, ComptrollerABI, provider)
    );
    ...
    console.log('=====>allComptrollerMarkets', allComptrollerMarkets);
